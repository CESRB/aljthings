      ...
      <div className="productLayout">
        <section className="gallery">
          ...
          <div className="galleryNote">
            Sample listing photo · Real item photos will replace this before launch
          </div>
        </section>
        <section className="productDetails">
          <p className="kicker purple">{item.category}</p>
          <h1>{item.name}</h1>
          <div className="priceLine">
            <strong>${item.price}</strong>
            <span className={"stock " + item.status}>{item.status}</span>
          </div>
          <p className="description">{item.description}</p>
          <div className="fulfillmentCard">
            {item.fulfillment === "pickup" ? <MapPin /> : <Truck />}
            <div>
              <b>{fulfillmentLabel(item.fulfillment)}</b>
              <span>
                {item.fulfillment === "pickup"
                  ? "Pickup details provided after purchase."
                  : "Final options and cost shown at checkout."}
              </span>
            </div>
          </div>
          ...
          {canBuy ? (
            <a className="buyButton" href={`/?add=${item.id}#finds`}>
              Add to bag
            </a>
          ) : (
          ...
          <p className="buyerNote">
            <ShieldCheck /> Each listing includes current photos and clear
            condition details.
          </p>
        </section>
      </div>
